    ...
    return (
      <div className="bg-white p-3 border border-slate-200 rounded-lg shadow-lg text-xs">
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <span className="font-bold ml-auto text-slate-800">{data.cumples} ({pctCumples}%)</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-slate-200"></div>
          <span className="text-slate-600">Pendiente/Fallo:</span>
          <span className="font-bold ml-auto text-slate-800">{data.fallas} ({pctFallas}%)</span>
        </div>
      </div>
    );
  }
      ...
      <CardContent>
        <div className="h-[300px] w-full">
          {!loading && data.length > 0 ? (
            ...
            <div className="h-full flex items-center justify-center text-muted-foreground bg-slate-50 dark:bg-slate-800/50 rounded-xl">
